...
"""
Walk one or more TNDS timetable files and emit to start and end of
their contained routes as dot files
"""
import os
import sys
...
def process(filename):
    ...
    for route in tree.findall('n:Routes/n:Route', ns):
      ...
      first_link = route_section.find("n:RouteLink[1]" , ns)
      last_link = route_section.find("n:RouteLink[last()]" , ns)

      # Assume all RouteLinks in a RouteSection have the same direction
      direction = first_link.find("n:Direction", ns).text
      from_stop = first_link.find('n:From/n:StopPointRef', ns).text
      to_stop = last_link.find('n:To/n:StopPointRef', ns).text
    ...
    for direction in graphs:
      print(graphs[direction].render('%s-%s-summary' % (basename,direction), cleanup=True))
